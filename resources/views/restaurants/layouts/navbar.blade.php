    ...
    <header class="header navbar navbar-expand-sm">
        ...
        <ul class="navbar-item theme-brand flex-row  text-center">
            ...
            <li class="nav-item theme-text">
                <a href="{{route('store_admin.dashboard')}}" class="nav-link">{{ Auth::user()->store_name }}</a>
            </li>
        </ul>
                        ...
                        <div class="dropdown-menu position-absolute">
                            @foreach($languages as $language)
                                @if(Session::get('selected_language')!=NULL)
                                    <option class="dropdown-item d-flex align-self-center" {{Session::get('selected_language') == $language->id ?"selected": null}} value="{{$language->id}}">
                                        {{$language->language_name}}
                                    </option>

                                @endif
                                @if(Session::get('selected_language')==NULL)
                                    <option class="dropdown-item d-flex align-self-center" {{$language->is_default == 1 ?"selected": null}} value="{{$language->id}}">
                                        {{$language->language_name}}
                                    </option>
                                @endif
                ...
                <div class="page-header">
                    ...
                    <nav class="breadcrumb-one" aria-label="breadcrumb">
                        <ol class="breadcrumb">
                            ...
                            <li class="breadcrumb-item active" aria-current="page"><span>{{$root_name}}</span></li>
                        </ol>
                    </nav>
